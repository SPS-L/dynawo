...
class LineInterfaceIIDM : public LineInterface, public boost::noncopyable {
  ...
  */
   ...
   */
  void hasConnectionSide1(bool hasConnection) override {hasConnectionSide1_ = hasConnection;}

  /**
   * @brief Getter for the line's connection attribute on side 1
   * @return @b true if the line has an outside connection on side 1, @b false else
   */
  bool hasConnectionSide1() const override {return hasConnectionSide1_;}

  /**
   * @brief Setter for line outside connection attribute side 2 (whether the bus this side has a dynamic model)
   * @param hasConnection @b true if the line has an outside connection this side, @b false else
   */
  void hasConnectionSide2(bool hasConnection) override {hasConnectionSide2_ = hasConnection;}

  /**
   * @brief Getter for the line's connection attribute on side 2
   * @return @b true if the line has an outside connection on side 2, @b false else
   */
  bool hasConnectionSide2() const override {return hasConnectionSide2_;}
 ...
 private:
  ...
  bool hasConnectionSide1_ = false;                 ///< @b true if the line has an outside connection (i.e, dynamic bus) side 1, @b false else
  bool hasConnectionSide2_ = false;                 ///< @b true if the line has an outside connection (i.e, dynamic bus) side 2, @b false else
