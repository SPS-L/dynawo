...
createModelLoad(bool open, bool initModel, powsybl::iidm::Network& networkIIDM) {
  ...
  std::shared_ptr<LoadInterfaceIIDM> loadItfIIDM = std::make_shared<LoadInterfaceIIDM>(loadIIDM);
...
class StubStateChangeComponent : public NetworkComponent {
 public:
  ...
  StateChange_t evalZ(double /*t*/) override { return NetworkComponent::NO_CHANGE; }
...
TEST(ModelsModelNetwork, ModelNetworkVoltageLevelNodeBreakerPromotionIsDowngradable) {
  ...
  // classified downgradable — non-switch events inside a voltage level
  // (loads, generators, shunts) therefore reach the same ALGEBRAIC_MODE
  // path as switch events (covered end-to-end by
...
TEST(ModelsModelNetwork, ModelNetworkLoadInitializationOpened) {
  powsybl::iidm::Network networkIIDM("MyNetwork", "MyNetwork");
  std::shared_ptr<ModelLoad> load = std::get<0>(createModelLoad(true, false, networkIIDM));
  ASSERT_EQ(load->id(), "MyLoad");
