...
TEST(ModelsModelNetwork, ModelNetworkLineCalculatedVariables) {
  ...
  ASSERT_THROW_DYNAWO(dl->getIndexesOfVariablesUsedForCalculatedVarI(42, numVars), Error::MODELER, KeyError_t::UndefCalculatedVarI);
...
TEST(ModelsModelNetwork, ModelNetworkLineDiscreteVariables) {
  ...
  z[0] = OPEN;
  z[1] = 0.;
  ASSERT_EQ(dl->evalZ(0., onlyEvaluateStateChange), NetworkComponent::TOPO_CHANGE);
  ASSERT_EQ(dl->getConnectionState(), OPEN);
  ASSERT_EQ(z[0], OPEN);
  ASSERT_DOUBLE_EQUALS_DYNAWO(dl->getCurrentLimitsDesactivate(), 0.);
  ASSERT_DOUBLE_EQUALS_DYNAWO(z[1], dl->getCurrentLimitsDesactivate());
  ...
  dl->setConnectionState(OPEN);
  ASSERT_EQ(dl->evalZ(0., onlyEvaluateStateChange), NetworkComponent::NO_CHANGE);
  ...
  ASSERT_THROW_DYNAWO(dl->evalZ(0., onlyEvaluateStateChange), Error::MODELER, KeyError_t::NoThirdSide);

  dl->setConnectionState(UNDEFINED_STATE);
  ASSERT_THROW_DYNAWO(dl->evalZ(0., onlyEvaluateStateChange), Error::MODELER, KeyError_t::UndefinedComponentState);
...
TEST(ModelsModelNetwork, ModelNetworkDynamicLine) {
  ...
  z3[0] = CLOSED_2;
  ASSERT_THROW_DYNAWO(dl3->evalZ(0, onlyEvaluateStateChange), Error::MODELER, KeyError_t::DynamicLineStatusNotSupported);
  z3[0] = CLOSED_1;
  ASSERT_THROW_DYNAWO(dl3->evalZ(0, onlyEvaluateStateChange), Error::MODELER, KeyError_t::DynamicLineStatusNotSupported);
  z3[0] = CLOSED;
  ASSERT_NO_THROW(dl3->evalZ(0, onlyEvaluateStateChange));
  delete[] zConnected;
  delete[] zConnected3;
}
...
TEST(ModelsModelNetwork, ModelNetworkLineDefineInstantiate) {
  ...
  for (int i = 0; i < 17; ++i) {
    std::string var = instantiatedVariables[instantiatedVariables.size()-i-1]->getName();
    boost::replace_all(var, dl->id(), "@ID@");
    ASSERT_EQ(definedVariables[definedVariables.size()-i-1]->getName(), var);
    ASSERT_EQ(definedVariables[definedVariables.size()-i-1]->getType(), instantiatedVariables[instantiatedVariables.size()-i-1]->getType());
  }
