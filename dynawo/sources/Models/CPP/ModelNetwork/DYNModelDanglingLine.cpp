...
modelType_("DanglingLine")  {
  ...
  if (!cLimit.empty()) {
    ...
    if (cLimit[0]->getLimit() < maximumValueCurrentLimit) {
      ...
      currentLimits_->addLimit(cLimit[0]->getName(), limit, cLimit[0]->getAcceptableDuration(), false);
    }
    for (unsigned int i = 1; i < cLimit.size(); ++i) {
      ...
      if (cLimit[i-1]->getLimit() < maximumValueCurrentLimit) {
        ...
        currentLimits_->addLimit(cLimit[i]->getName(), limit, cLimit[i]->getAcceptableDuration(), false);
      }
    }
    for (unsigned int i = 1; i < cLimit.size(); ++i) {
      if (!cLimit[i]->isFictitious()) continue;
      if (cLimit[i]->getLimit() < maximumValueCurrentLimit) {
        ...
        currentLimits_->addLimit(cLimit[i]->getName(), limit, cLimit[i]->getAcceptableDuration(), true);
      }
    }
  }
...
ModelDanglingLine::evalZ(const double t, bool onlyEvaluateStateChange) {
  if (currentLimits_ && !onlyEvaluateStateChange) {
