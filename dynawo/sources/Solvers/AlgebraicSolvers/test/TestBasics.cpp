...
TEST(AlgebraicSolvers, testInit) {
  ...
  constexpr bool printResiduals = false;
  // KINSetFuncNormTol
  ASSERT_THROW_DYNAWO(solverEuler->init(model, timeSchemeSolver, -1, 1, 1, 1, 1, 1, 1, sundialsVectorY, printResiduals), Error::SUNDIALS_ERROR,
  ...
  ASSERT_THROW_DYNAWO(solverEuler->init(model, timeSchemeSolver, 1, 1, 1, 1, 1, 1, -1, sundialsVectorY, printResiduals), Error::SUNDIALS_ERROR,
                      KeyError_t::SolverFuncErrorKINSOL);
  ASSERT_NO_THROW(solverEuler->init(model, timeSchemeSolver, 1, 1, 1, 1, 1, 1, 1, sundialsVectorY, printResiduals));
...
TEST(AlgebraicSolvers, testModifySettings) {
  ...
  std::shared_ptr<Model> model = initModelFromDyd("dyd/solverTestAlpha.dyd");
  boost::shared_ptr<SolverKINAlgRestoration> solver(new SolverKINAlgRestoration(printReinitResiduals));
  ...
  boost::shared_ptr<SolverKINAlgRestoration> solver2(new SolverKINAlgRestoration(printReinitResiduals));
