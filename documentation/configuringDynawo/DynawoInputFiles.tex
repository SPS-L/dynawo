...
busCriteria, loadCriteria, generatorCriteria, quadripoleCriteria}, deletekeywords={type}, title=crt
...
criterion (except for quadripole craterion for which an empty list will result in ignoring the corresponding criteria).
If some components id are specified then only those components will
be subject to this criterion. Components from the list should have the same type as the
specified type (buses, loads, quadripoles (i.e. lines or transformers) or generators).
...
It is triggered when the following condition happens for any of the bus: \\
\begin{center}
$U_{bus} < UMinPu*U_n$\\
and/or\\
$U_{bus} > UMaxPu*U_n$ \\
...
If local values was selected  (parameter type = 'LOCAL\_VALUE'), it is triggered when
...
$P_{source} > PMax$\\
\end{center}
If sum was selected  (parameter type = 'SUM'), it is triggered when the
...
\subsection{quadripole criteria}

Quadripole criteria applies to lines or two-winding transformers whose
ids are specified in the componenent list. It
should have $i1Max$ or $i2Max$ or both specified (in A). Only
local values can be checked (parameter type = 'LOCAL\_VALUE').
It is triggered when the following condition happens for any of the specified
quadripoles: \\
\begin{center}
$I_{quadripole,side_1} > i1Max$\\
and/or\\
$I_{quadripole,side_2} > i2Max$\\
\end{center}
...
\subsubsection{Track the real time performance of the simulation}

The user can ask \Dynawo to record the wall-clock duration of each simulation step by setting the ``enableRealTimeTracking'' attribute of the ``simulation'' item to ``true''. This attribute is optional and disabled by default: when it is not set (or set to ``false''), no timing data is collected and no additional file is written.

\begin{lstlisting}[language=XML, morekeywords={enableRealTimeTracking}]
<job name="MyJOB">
  ...
  <simulation startTime="0" stopTime="100" precision="1e-6" enableRealTimeTracking="true">
    <criteria criteriaFile="criteria.crt"/>
  </simulation>
  ...
</job>
...
When enabled, \Dynawo writes the file \path{simRT.csv} in the outputs directory at the end of the simulation. Each row corresponds to one simulation step and contains three columns:
\begin{itemize}
  \item \texttt{simulation\_time} — the simulation time, in seconds, reached at the end of the step;
  \item \texttt{computation\_time\_ms} — the wall-clock duration of that step alone, in milliseconds;
  \item \texttt{accumulated\_computation\_time\_s} — the wall-clock time, in seconds, elapsed since the start of the simulation loop, up to and including that step.
\end{itemize}

The third column is measured independently from the second: it is the elapsed wall-clock time since the simulation loop began, not the running sum of \texttt{computation\_time\_ms}. The two are expected to diverge whenever work happens between steps outside of what is being timed as the step itself, for instance intermediate state dumps; comparing them is a way to see how much of the total run time such inter-step work accounts for.
...
Constraints can be filtered at export with the ``filter'' flag (default=``FIRST'').

If filter=``DISABLED'', all constraints registered during the simulation will be kept, even if the constraint was cleared before the end of the simulation.

If filter=``FIRST'', constraints cleared during simulation will be removed and the ones still existing at the end of the simulation are associated to the value seen at the first time step when the constraint appeared.

If filter=``DYNAFLOW'', constraints cleared during simulation will be kept and will contain the last value available (but timestep of the first one), whether that corresponds to the end of the constraint or the end of the simulation. Additionally, the maximum value observed while the constraint was in effect will be exported for each constraint.

\begin{lstlisting}[language=XML, morekeywords={constraints},numbers=none]
<constraints exportMode="TXT" filter="FIRST"/>
\end{lstlisting}
Three export modes are available for constraints: TXT, XML or JSON.
...
\end{lstlisting}
Four export modes are available for the timeline: TXT, XML, CSV or JSON.
...
Three export modes are available for curves export: CSV, XML or HDF5.
It's impossible to define both an iterationStep and a timeStep.

The \textbf{HDF5} export mode writes a single binary file \path{curves/curves.h5} in the outputs directory.
The file contains three datasets:
\begin{itemize}
  \item \texttt{/time} — 1-D array of simulation time values (one entry per recorded time step);
  \item \texttt{/data} — 2-D array of shape \textit{(time steps} $\times$ \textit{curves)}, where each column corresponds to one requested curve;
  \item \texttt{/curve\_names} — string attribute on \texttt{/data} listing the curve identifiers in column order.
\end{itemize}

\begin{lstlisting}[language=XML, morekeywords={curves},numbers=none]
<curves inputFile="MyCurves.crv" exportMode="HDF5"/>
\end{lstlisting}

HDF5 export requires \Dynawo to have been compiled with HDF5 support (CMake option \texttt{DYNAWO\_ACTIVATE\_HDF5\_EXPORT}, enabled by default).
If HDF5 support is not available and \texttt{exportMode="HDF5"} is requested, \Dynawo will raise an error at runtime.

To visualise HDF5 curves, the \texttt{curves-visu} command of \path{envDynawo.sh} automatically detects HDF5 export mode and launches an interactive Streamlit viewer:
\begin{lstlisting}[language=bash,numbers=none]
./envDynawo.sh curves-visu path/to/simulation.jobs
\end{lstlisting}
The viewer requires the Python packages \texttt{h5py}, \texttt{streamlit} and \texttt{plotly}.
They can be installed with:
\begin{lstlisting}[language=bash,numbers=none]
python -m pip install -r $DYNAWO_CURVES_VISU_APP_DIR/requirements.txt
\end{lstlisting}
...
\subsubsection{Specify the interactive mode settings}

To run \Dynawo in interactive mode, settings needs to be added to the jobs file.

\begin{lstlisting}[language=XML, morekeywords={interactiveSettings}]
<job name="MyJOB">
  ...
    <interactiveSettings couplingTimeStep="1">
        <channels>
            <channel id="zmq_input" kind="INPUT" type="ZMQ" endpoint="tcp://localhost:5555"/>
            <channel id="zmq_output" kind="OUTPUT" type="ZMQ" endpoint="tcp://*:5556"/>
        </channels>
        <clock type="EXTERNAL" triggerChannel="zmq_input"/>
        <streams>
            <stream data="CURVES" channel="zmq_output" format="BYTES"/>
            <stream data="TIMELINE" channel="zmq_output" format="JSON"/>
            <stream data="CONSTRAINTS" channel="zmq_output" format="JSON"/>
        </streams>
    </interactiveSettings>
</job>
\end{lstlisting}

The interactiveSettings attribute 'couplingTimeStep' corresponds to the minimum time interval in seconds between two coupling phase. A coupling phase consists of outputting the results, then waiting for an internal or external signal before continuing the simulation.

Subelements of interactiveSettings tag are mandatory:

\begin{itemize}

\item \textbf{channels}: channels is a set of channel elements. A channel element defines the input channels (reception of actions, time management signals) or output channels (publication of curves, timeline, constraints). Each channel has a unique id, and a kind (INPUT or OUTPUT).
...
\midrule
\rowcolor{white}
\textbf{id} & string & Identifier of the channel \\
\rowcolor{gray!10}
\textbf{kind} & string & Kind of channel: INPUT or OUTPUT \\
\rowcolor{white}
\textbf{type} & string & Type of channel: ZMQ \\
\rowcolor{gray!10}
\textbf{endpoint} & string & specification of the endpoint \\
\bottomrule
\end{tabular}
\caption{calculatedhannel parameters}
\end{table}

Currently implemented channels are:
\begin{itemize}
  \item \textbf{INPUT/ZMQ}: ZeroMQ interface in 'reply' mode on specified endpoint (default = 'tcp://*:5555'). Needs ZeroMQ library installed.
  \item \textbf{OUTPUT/ZMQ}: ZeroMQ interface in 'pub' mode on specified endpoint  (default = 'tcp://*:5556'). Needs ZeroMQ library installed.
\end{itemize}

\item \textbf{streams}: streams is a set of stream elements. A stream defines the publication of a type of data using a specified output channel. Data will be published at a rate defined by the 'couplingTimeStep' attribute. Each stream shall be defined using the attributes:

\begin{itemize}
  \item \textbf{data}: type of data: CURVES, CONSTRAINTS, TIMELINE or LOGS
  \item \textbf{channel}: output channel to publish to, identified by its id.
  \item \textbf{format}: format for the output data, depending on the data type:
  \begin{itemize}
    \item for \textbf{CURVES}: JSON, CSV on the 'curves' topic, or BYTES which sends at the begining of the simulation the variables names separated by a newline character to topic "curves\_names, then periodically values as a vector of raw bytes to topic 'curves\_values'.
    \item for \textbf{CONSTRAINTS}: JSON, TXT or XML to topic 'constraints'
    \item for \textbf{TIMELINE}: JSON, CSV, TXT or XML to topic 'timeline'
  \end{itemize}

\end{itemize}

\item \textbf{clock}: clock element define how the time is synchronized with the environment.

Two clock types are defined:
\begin{itemize}
  \item \textbf{Internal clock}: simulation loop is waiting at the end of each coupling time step, to remain synchronized with the computer clock. A speedup factor can be applied to differenciate the clocks.
  \item \textbf{External clock}: simulation loop waits for an external trigger at the end of each coupling time step. This trigger is a specific signal received though an input channel.
\end{itemize}
...
\midrule
\rowcolor{white}
\textbf{type} & string & INTERNAL or EXTERNAL \\
\rowcolor{gray!10}
\textbf{speedup} & double & Speedup factor for the internal clock \\
\rowcolor{white}
\textbf{triggerChannel} & string & Trigger input channel id for the external clock \\
\bottomrule
\end{tabular}
...
<macroConnector id="M1S_OMEGAREF_CONNECTOR">
  ...
  <connect var1="running_grp_@INDEX@_value" var2="M1S_running"/>
...
\Dynawo can be built with an internal timing instrumentation that measures how much wall-clock time is spent in the main phases of the simulation and of the algebraic and differential solvers. It is controlled by the CMake option \texttt{DYNAWO\_TIMERS}, which is \texttt{OFF} by default. When it is set to \texttt{ON}, \Dynawo is built with the compile-time macro \texttt{PRINT\_TIMERS} defined, which activates the instrumentation. A reader who prefers not to pass a CMake option can instead uncomment the line \texttt{// \#define PRINT\_TIMERS} near the top of \texttt{DYNTimer.h} and rebuild: both routes define the same macro and have the same effect. When the instrumentation is disabled, which is the default either way, every instrumentation point compiles to nothing, so a default build pays no cost for it, not even a clock read.

\subsection{The timed phases}

When enabled, the instrumentation times 17 phases of the simulation and solver code. A phase can be nested inside another phase, in which case the outer phase is its parent. The phases are:

\begin{itemize}
  \item \texttt{SimulationLoop}: the main time-stepping loop, from the first step to the end of the run. It does not include the initial condition calculation, which happens earlier;
  \item \texttt{SolverSolve}: one call to the solver's per-step entry point, which resets the step state, reinitializes the model's mode and calls the solver's step function;
  \item \texttt{CalculateIC}: the computation of consistent initial conditions, performed once during initialization, before the simulation loop starts;
  \item \texttt{SolverStep}: the numerical integration of one time step, whether by a fixed time step solver (SIM, TRAP) or by a variable time step solver (IDA);
  \item \texttt{JacobianEval}: one Jacobian evaluation callback handed to the SUNDIALS solver. This callback is shared by more than one solver (algebraic restoration, implicit Euler and, separately, IDA), so this phase aggregates calls coming from several places in the code;
  \item \texttt{ResidualEval}: one residual, that is equation system, evaluation callback handed to the SUNDIALS solver, aggregated the same way as \texttt{JacobianEval};
  \item \texttt{RootEval}: evaluation of the root, that is discrete event detection, functions: IDA's root-finding callback handed to SUNDIALS, and, for the fixed time step solvers, the related bookkeeping performed before, during and after a step;
  \item \texttt{ModeEval}: evaluation of the model's mode;
  \item \texttt{DiscreteEval}: evaluation of the model's discrete variables;
  \item \texttt{NRSolve}: the Newton-Raphson style algebraic solve performed once per step by the fixed time step solvers;
  \item \texttt{MatrixCopy}: copying \Dynawo's sparse Jacobian into the SUNDIALS matrix structure;
  \item \texttt{KINSOLSolve}: the call to the SUNDIALS KINSOL nonlinear solve itself;
  \item \texttt{Reinit}: the algebraic-restoration driver, run whenever a mode change during the run requires a new algebraic restoration, for both the fixed time step solvers and IDA. It covers the whole driver, not only (re)configuring the KINSOL instance used for the restoration: buffer rotation, that configuration step, the KINSOL solve itself, and retries and statistics. The configuration step is entered again inside the driver as a child of itself and, by the self-nesting rule below, is not counted a second time; it is also entered on its own, with no enclosing driver, once during initialization from the initial condition calculation;
  \item \texttt{IO}: writing the simulation's final output, at the end of the run;
  \item \texttt{KLUSymbolic}: the KLU symbolic refactorization, triggered whenever the Jacobian's sparsity pattern changes;
  \item \texttt{KLUSetup}: the KLU numeric factorization performed by the SUNDIALS linear solver;
  \item \texttt{CurvesUpdate}: updating the values of the curves selected for export.
\end{itemize}

\subsubsection{A caveat on phases reached from local model initialization}

Several phases are reached not only from the main simulation and solver code but also from the KINSOL-based solver used for the local algebraic initialization of each submodel; a call reached this way gets no parent, since that local initialization is outside the set of phases instrumented today, so it is measured as a root rather than nested under \texttt{JacobianEval} or another enclosing phase. \texttt{MatrixCopy}, \texttt{KLUSymbolic} and \texttt{KLUSetup} are affected because the routine that copies the Jacobian and triggers the symbolic refactorization, and the KLU setup wrapper installed on every KINSOL instance the solvers create, are both reached this way as well as from the main solvers. \texttt{KINSOLSolve} is affected by the same mechanism, through that solver's own call to the shared KINSOL-solve routine, and is the largest of the phases affected by it. \texttt{ResidualEval} is not affected: that solver's own residual callback is not instrumented. \texttt{Reinit} is not affected either: local model initialization does not go through the algebraic-restoration driver that phase measures.

In other words, some \texttt{MatrixCopy}, \texttt{KLUSymbolic}, \texttt{KLUSetup} and \texttt{KINSOLSolve} calls happen with no corresponding \texttt{JacobianEval} call above them. On the Nordic test system this shows up as 58 \texttt{MatrixCopy} calls against 38 \texttt{JacobianEval} calls: a reader comparing those two counts should not read the difference as a bug in the instrumentation.

\subsection{Inclusive and exclusive time}

For each phase, the instrumentation records an inclusive time, the total time spent in the phase including everything called from within it, and an exclusive time. The exclusive time is the inclusive time with every child phase's time subtracted, not a chosen subset of children: whatever time was spent inside any other instrumented phase while this one was active is removed, all of it.

\Dynawo has more than one entry point with no active parent phase: for instance, the initial condition calculation (\texttt{CalculateIC}) is performed during initialization, before the simulation loop is entered, so a run has at least two independent top-level phases, \texttt{CalculateIC} and \texttt{SimulationLoop}, not one. Because of this, the identity that holds is: \textbf{the sum of every phase's exclusive time equals the sum of every measurement that was recorded with no parent phase}. It is not the same as the total time of \texttt{SimulationLoop} alone, and should not be compared against it. The trace report described below prints both quantities on a trailing line, together with two figures that can actually flag a problem: whether an indirect-recursion cycle was detected, and how many phases have a negative exclusive time (deliberately not clamped to zero, since it is the only surviving signal that a phase's attribution broke). The exclusive/root identity itself holds by construction, every measurement is added to exactly one of the two totals, so it is informational only; its presence on that line is not a check that a run's own numbers are self-consistent.

A phase nested inside itself, directly or through a chain of calls that returns to it, is counted once: only the time of its outermost instance is recorded, so a self-nested call neither doubles the phase's total time nor appears as its own child. Indirect recursion, meaning a phase re-entered while it is already open somewhere on the call stack but is not the innermost open phase, for instance a cycle where phase A calls phase B which calls phase A again, is a different case: it is detected and reported with a one-shot warning, because the exclusive time of the phases involved in such a cycle can no longer be attributed correctly.

\subsection{The CSV output}

When the instrumentation is enabled, setting the environment variable \texttt{DYNAWO\_TIMERS\_OUTPUT} to a file path makes \Dynawo write the phase statistics to that file, in CSV format, once the run ends. If the variable is not set, no such file is written. Phases that were never entered during the run are omitted from the file.

The columns, in order, are \texttt{phase}, \texttt{total\_seconds}, \texttt{exclusive\_seconds}, \texttt{call\_count}, \texttt{min\_ms} and \texttt{max\_ms}. \texttt{total\_seconds} and \texttt{exclusive\_seconds} are expressed in seconds; \texttt{call\_count} is a plain, unitless count of how many times the phase was entered; \texttt{min\_ms} and \texttt{max\_ms}, the shortest and longest single measurement recorded for the phase, are expressed in milliseconds.

The same information, together with the trailing line described above, is also written as a short table to the simulation log whenever at least one phase was entered. This happens once per job, at the end of that job's own \texttt{terminate()}, so a jobs file that defines more than one job gets one table per job rather than one table aggregated across all of them.

\subsubsection{Reading the output: a worked example}

The following is an excerpt of the CSV file produced by a run of the Nordic test system with the instrumentation enabled:

\begin{lstlisting}[language=bash,numbers=none]
phase,total_seconds,exclusive_seconds,call_count,min_ms,max_ms
JacobianEval,0.053047,0.052833,38,1.1750,1.8650
MatrixCopy,0.000146,0.000146,58,0.0000,0.0170
KLUSymbolic,0.000070,0.000070,53,0.0000,0.0170
KLUSetup,0.016934,0.016934,58,0.0040,0.6470
\end{lstlisting}

\texttt{JacobianEval} was entered 38 times for a total of about 53.0 ms, of which 52.8 ms is exclusive: a small remainder, about 0.2 ms, was spent inside other instrumented phases, \texttt{MatrixCopy} and \texttt{KLUSymbolic} among them, while a Jacobian evaluation was on the call stack. \texttt{MatrixCopy} and \texttt{KLUSymbolic} are themselves cheap and have no instrumented children of their own, so for both of them the total and exclusive columns are equal. Both were also called more often than \texttt{JacobianEval}, 58 and 53 times against 38: the extra calls are the ones described in the caveat above, reached from local algebraic initialization rather than from a Jacobian evaluation. \texttt{KLUSetup}, called from yet another site, the SUNDIALS linear solver's own setup routine, accounts for about 16.9 ms in total across its 58 calls, roughly a third of \texttt{JacobianEval}'s total; its mean call is therefore about 0.29 ms, well under its maximum of 0.647 ms, so most calls are cheap and the cost is concentrated in a few longer ones.
...
Connected variables should share the same type (basically Boolean, Integer, continuous Real or discrete Real). For simplification purposes and in order to identify easily which are the interface variables of the model, the current \Dynawo Modelica library uses causal connectors from the Modelica Standard Library: BooleanInput or BooleanOutput, IntegerInput or IntegerOutput, RealInput or RealOutput (continuous, unless preceded by the keyword discrete). Moreover, the library uses a non-causal connector: ACPower whose attributes are a complex voltage V (potential variable) and a complex current i (flow variable). \\

For example, if we connect an RealOutput connector (let's say SP\_setPoint) to a BooleanInput connector (Generator\_running), we will get the following error message:
...
Mistakes in variables names in the crv file won't trigger any error message, but simply won't be taken into account in the generation of the output curves file (\path{curves.csv}, \path{curves.xml} or \path{curves.h5} depending on the export mode). If it happens that you can't find the variable that was supposed to figure in the curves output, check that it is written correctly.
