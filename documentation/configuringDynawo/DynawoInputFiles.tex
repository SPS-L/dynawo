...
\begin{itemize}
  ...
  \item \texttt{Reinit}: (re)configuring the KINSOL instance used for algebraic restoration, both once during initialization and again whenever a mode change during the run requires a new algebraic restoration;
...
\subsubsection{A caveat on the KLU-related phases}

\texttt{MatrixCopy}, \texttt{KLUSymbolic} and \texttt{KLUSetup} also include work performed during model initialization that is not accounted for under \texttt{JacobianEval}. The routine that copies the Jacobian and triggers the symbolic refactorization is also reached from the code that performs the local algebraic initialization of each submodel, which is outside the set of phases instrumented today; the KLU setup wrapper, in turn, is installed on every KINSOL instance created by the solvers, including the one used for that local initialization. In other words, some \texttt{MatrixCopy}, \texttt{KLUSymbolic} and \texttt{KLUSetup} calls happen with no corresponding \texttt{JacobianEval} call above them. On the Nordic test system this shows up as 58 \texttt{MatrixCopy} calls against 38 \texttt{JacobianEval} calls: a reader comparing those two counts should not read the difference as a bug in the instrumentation.
...
The same information, together with the identity described above, is also written as a short table to the simulation log whenever at least one phase was entered.
